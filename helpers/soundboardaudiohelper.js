...
class SBAudioHelper {
    ...
    async play({src, volume, detune}, sound) {
        ...
        // NEVER set soundNode.loop = true when using SoundBoard loop modes.
        // soundNode.loop = true causes the Web Audio API to loop internally,
        // bypassing the 'end' event and making delay/random impossible.
        // SoundBoard always manages looping manually via the 'end' event.
        ...
        soundNode.addEventListener('end', () => {
            ...
            if (sound?.loop) {
                // Compute delay: fixed, random, or none
                let delayMs = 0;
                if (sound.loopMode === 'random') {
                    const min = (sound.loopDelayMin || 0) * 1000;
                    const max = (sound.loopDelayMax || 0) * 1000;
                    delayMs = min + Math.random() * (max - min);
                } else if (sound.loopMode === 'fixed') {
                    delayMs = (sound.loopDelayMin || sound.loopDelay || 0) * 1000;
                }
                // loopMode not set = immediate repeat (delayMs stays 0)
