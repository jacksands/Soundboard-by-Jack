...
class SBPlayerDirectoryManager extends foundry.applications.api.ApplicationV2 {
    ...
    async _renderHTML(context, options) {
        ...
        const playerRows = context.players.map(player => {
            // S車 GM pode editar qualquer campo, jogador s車 pode editar o pr車prio
            const editable = isGM;
            return `
    ...
    _attachListeners() {
        ...
        html?.querySelectorAll('.sb-player-picker-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                // S車 GM pode editar qualquer campo
                if (!isGM) return;
        ...
        html?.querySelectorAll('.sb-player-save-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                const playerId = e.target.dataset.playerId;
                // S車 GM pode salvar qualquer campo
                if (!isGM) return;
                const input = html.querySelector(`.player-directory-input[data-player-id="${playerId}"]`);
                if (input) {
...
class SoundBoard {
    ...
    static handlebarsHelpers = {
                'is-playing': function(identifyingPath) {
                    // Verifica se o som est芍 tocando
    ...
    // Settings client-scope podem ser lidos/escritos por qualquer usu芍rio.
    // Settings world-scope restricted s車 o GM pode escrever (Foundry j芍 bloqueia).
    ...
    static async openSoundBoard() {
        // S車 permite abrir se for GM ou se a op??o permitir
        ...
        // Garante que pode abrir m迆ltiplas vezes
    ...
    static getVolume() {
        // Usa o novo setting client-side para volume geral do m車dulo
    ...
    static async playSound(identifyingPath, push = true) {
        ...
        if (!game.user.isGM) {
            if (push) {
                // Resolve qual arquivo tocar (suporte a wildcard/m迆ltiplos src)
        ...
        // Adiciona 角 lista de sons tocando e atualiza indicador visual
    ...
    static startLoop(identifyingPath, loopMode = 'simple', delayMin = 0, delayMax = 0) {
        ...
        sound.loopDelay = 0;
        SoundBoard.playSound(identifyingPath);
        SoundBoard._applyLoopActiveDOM(identifyingPath, true);
    }

    /**
     * Aplica/remove loop-active no DOM e expande a categoria se necess芍rio.
     * Funciona mesmo quando o board est芍 fechado (idempotente).
     */
    static _applyLoopActiveDOM(identifyingPath, active) {
        const escaped = CSS.escape(identifyingPath);
        document.querySelectorAll(`#soundboard-app .btn[uuid="${escaped}"]`).forEach(btn => {
            btn.classList.toggle('loop-active', active);
            if (active) {
                const collapseDiv = btn.closest('.sb-collapse-div');
                if (collapseDiv && collapseDiv.style.display === 'none') {
                    collapseDiv.style.display = '';
                }
            }
        });
        SoundBoard._updateCategoryHeaderIcons();
    }

    /**
     * Varre todos os headers de categoria e atualiza o 赤cone de som ativo.
     */
    static _updateCategoryHeaderIcons() {
        document.querySelectorAll('#soundboard-app #soundboard-filterable .sb-collapse-header').forEach(header => {
            ...
            if (!div) return;
            const hasActive = div.querySelector('.btn[uuid].loop-active') !== null;
            let icon = header.querySelector('.sb-cat-active-icon');
            if (hasActive) {
                if (!icon) {
                    icon = document.createElement('i');
                    icon.className = 'fas fa-volume-up sb-cat-active-icon';
                    icon.title = 'Som ativo nesta categoria';
    ...
    /**
     * Reaplica loop-active e sb-playing ap車s o board ser renderizado/aberto.
     */
    static restoreActiveStates() {
        ...
        Object.values(allSounds).forEach(arr => arr.forEach(s => {
            if (s.loop) SoundBoard._applyLoopActiveDOM(s.identifyingPath, true);
        }));
        SoundBoard.currentlyPlayingSounds.forEach(s => {
            if (s) SoundBoard._updatePlayingIndicator(s.identifyingPath, true);
        });
        SoundBoard._updateCategoryHeaderIcons();
    }
    ...
    static stopLoop(identifyingPath) {
        ...
        if (!game.user.isGM) {
            // Jogador: para 芍udio local e pede ao GM para cancelar o loop dele
        ...
        // GM: cancela loop local, para 芍udio e notifica todos os clientes
    ...
    static stopSound(identifyingPath) {
        let sound = SoundBoard.getSoundFromIdentifyingPath(identifyingPath);
        // Garante que o loop 谷 cancelado antes de parar o 芍udio
        sound.loop = false;
    ...
    static getDirectoryForCurrentUser() {
        if (game.user.isGM) {
            ...
            // Player: s車 pode usar o diret車rio definido pelo GM para ele, ou o padr?o do GM
            const playerDirs = game.settings.get('Soundboard-by-Jack', 'soundboardPlayerDirectories') || {};
    ...
    static async getSounds(forceRefresh = false) {
        ...
        console.log('SoundBoard: Usando diret車rio', soundboardDir);
    ...
    static _registerSettings() {
                        // Prefer那ncias do usu芍rio (client)
                        ...
                        game.settings.register('Soundboard-by-Jack', 'volumeToggle', {
                            ...
                            hint: 'Ativa/desativa o bot?o de volume r芍pido.',
                            scope: 'client',
                            config: false,
                            type: Boolean,
                            default: true
                        ...
                        game.settings.register('Soundboard-by-Jack', 'moduleGeneralVolume', {
                            name: 'Volume geral do m車dulo SoundBoard',
        ...
        // Menu para gerenciar diret車rios dos players (apenas GM)
        ...
        game.settings.register('Soundboard-by-Jack', 'soundboardDirectory', {
            ...
            onChange: value => {
                ...
                // For?a refresh para garantir persist那ncia e recarregar sons
    ...
    // Carregar sons de um diret車rio espec赤fico (sem alterar SoundBoard.sounds global).
    // Espelha a l車gica do getSounds principal: browse n?o-recursivo, categoria por subdir.
    ...
    // Sincronizar sons de um jogador espec赤fico (usado no ready e no userConnected)
    static async syncSoundsForPlayer(playerId) {
        ...
        if (playerDir && playerDir.trim()) {
            ...
            if (totalSounds === 0) {
                console.warn(`SoundBoard: Diret車rio "${playerDir}" retornou 0 sons para ${player.name} 〞 usando sons do GM como fallback.`);
                playerSounds = SoundBoard.sounds;
            }
        } else {
            // Sem diret車rio espec赤fico: usa os sons do GM como fallback
            console.log(`SoundBoard: Sem diret車rio para ${player.name} 〞 enviando sons do GM.`);
            playerSounds = SoundBoard.sounds;
        }
    ...
    static async syncPlayerSoundsToAll() {
        ...
        console.log('SoundBoard: Sincroniza??o de sons de jogadores conclu赤da');
    }
...
Hooks.on('renderSettingsConfig', (app, element) => {
    ...
    // V14: procura se??o/categoria do m車dulo e esconde tudo dela
    ...
    // Esconde cabe?alhos de se??o do m車dulo (V14 usa <section data-category="...">)
    ...
    el.querySelectorAll('h2, h3, .settings-header').forEach(h => {
        if (/soundboard/i.test(h.textContent)) {
            let el2 = h;
            // Remove o heading e todos os form-groups seguintes at谷 o pr車ximo heading
            const toRemove = [el2];
